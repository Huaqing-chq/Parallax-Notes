import type { AuthLayoutProps } from "@/features/theme/contract/layouts";
import { m } from "@/paraglide/messages";
import { AmbientBackground } from "../components/ambient-background";

export function AuthLayout({ onBack, children }: AuthLayoutProps) {
  return (
    <div className="windglass-theme relative isolate min-h-screen w-full flex flex-col">
      <AmbientBackground />
      <header className="h-16 flex items-center px-6 md:px-12">
        <button
          onClick={onBack}
          type="button"
          className="text-[10px] font-medium uppercase tracking-widest text-foreground/40 hover:text-foreground transition-colors wg-pressable"
        >
          ← {m.auth_layout_back_home()}
        </button>
      </header>

      <main className="flex-1 flex flex-col justify-center items-center p-6 md:p-12">
        <div className="w-full max-w-sm animate-in fade-in duration-500">
          <div className="wg-glass rounded-3xl p-8">{children}</div>
        </div>
      </main>

      <footer className="h-16" />
    </div>
  );
}
